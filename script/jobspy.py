...
class Job:
    ...
    def get_job_type(self):
        try:
            job_type = self.job.xpath(
                '//div[@class="metadata"]/div[@data-testid="attribute_snippet_testid"][not(ancestor::div[@class="metadata salary-snippet-container"])]/text()')[
                0].strip()
        except IndexError:
...
def scrape_jobs(db, browser, job_keyword, location_keyword, job_search_radius):
    ...
    for job in all_jobs:
        ...
        print(f"Job Description: {job_desc[:100]}...")  # printing first 50 characters
        time.sleep(1)
        record = (post_date, job_link, job_title, job_location, company_name, salary, job_desc, job_type, job_keyword,
                  datetime.now())
...
def main():
    job_search_keyword = ['Software', 'Embedded', 'AI', 'BizOps', 'Developer', 'Machine+Learning', 'Data+Analyst',
                          'Robotic', 'Cloud', 'DevOps', 'Data+Science']
    location_search_keyword = ['St. Louis', 'Seattle', 'Austin', 'Boston', 'Toronto', 'New York', 'Los Angeles',
    ...
    questions = [
        {
            'type': 'checkbox',
            'qmark': '🔍',
            'message': 'Select job positions',
            'name': 'positions',
            'choices': [{'name': job} for job in job_search_keyword],
            'validate': lambda answer: 'You must choose at least one position.' if len(answer) == 0 else True
        },
        {
            'type': 'checkbox',
            'qmark': '📍',
            ...
            'name': 'locations',
            'choices': [{'name': location} for location in location_search_keyword],
            'validate': lambda answer: 'You must choose at least one location.' if len(answer) == 0 else True
        },
        {
            'type': 'input',
            'name': 'radius',
            'message': 'Enter job search radius in miles:',
            'validate': lambda value: value.isdigit() or 'Please enter a valid number'
        }
    ]

    answers = prompt(questions)
    job_keywords = answers['positions']
    location_keywords = answers['locations']
    job_search_radius = int(answers['radius'])
